<div align="center">
  ...
  <div align="center" style="max-width: 680px; margin: 1.25rem auto 0; padding: 20px 22px 22px; border: 1px solid #d1d9e0; border-radius: 16px;">
    ...
    <p style="margin: 0 0 18px; padding-bottom: 16px; border-bottom: 1px solid #eaeef2; line-height: 2;">
      ...
      <span style="color: #d8dee4;"> &nbsp;·&nbsp; </span>
      <a href="https://www.fractal.com"><strong>공식 사이트</strong></a>
      <span style="color: #d8dee4;"> &nbsp;·&nbsp; </span>
...
> Fractal는 **셀프호스트·로컬 우선** 퀀트 **OS**입니다. 매수 버튼 챗봇이 아니라 **멀티 LLM 리서치**, **Python 네이티브 전략**, **서버 사이드 백테스트**, **멀티 브로커 라이브**(10+ crypto venue, IBKR, MT5, Alpaca)를 하나의 프로덕션급 스택에 통합합니다.

<div align="center">
  <img src="screenshots/ezgif.com-animated-gif-maker.gif" alt="Fractal 빠른 데모" width="920" style="border-radius: 12px; border: 1px solid #eaeef2;">
  <p><sub><em>제로에서 실행까지—차트, AI 리서치, 전략 워크플로를 몇 분 만에.</em></sub></p>
</div>

<div align="center">
  <img src="screenshots/architecture.png" alt="Fractal 아키텍처" width="960">
...
| **Agent 네이티브** | **Agent Gateway** + PyPI [`fractal-mcp`](https://pypi.org/project/fractal-mcp/) — Cursor / Claude Code / Codex, 감사 로그. |
...
기본 `~/fractal`. 재실행 시 최신 이미지 pull. → **`http://localhost:8888`** (`fractal` / `123456`, 즉시 비밀번호 변경).
...
브라우저에서 **`http://localhost:8888`** → **`fractal` / `123456`** 로그인 후 **관리자 비밀번호를 즉시 변경**하세요. 자세한 내용은 [설치 및 첫 실행](#설치-및-첫-실행)을 참고하세요.

## 관련 저장소

| 저장소 | 내용 |
|--------|------|
| **[Fractal](https://github.com/brokermr810/Fractal)** (본 저장소) | 백엔드, Compose, 문서, 프리빌드 Web |
| **[Fractal-Vue](https://github.com/brokermr810/Fractal-Vue)** | **Web 프론트 소스**(Vue) — `v*` 태그가 `ghcr.io/brokermr810/fractal-frontend`를 자동 발행 |
| **[Fractal-Mobile](https://github.com/brokermr810/Fractal-Mobile)** | **모바일 클라이언트**(오픈소스) |
...
- MCP 서버: [`../mcp_server/README.md`](../mcp_server/README.md) · PyPI [`fractal-mcp`](https://pypi.org/project/fractal-mcp/)
...
**설계 원칙:** 시세·전략/백테스트·실행 분리. Nginx + Vue SPA, Flask + Gunicorn, PostgreSQL 16, Redis 7. 배포: `install.sh` 한 줄, GHCR zero-repo, full repo Compose, AWS AMI, [SaaS](https://ai.fractal.com).
...
3. `docker-compose up -d --build`
   - **대안 (저장소 불필요)**: backend + frontend 모두 GHCR의 프리빌드 멀티 아키텍처(amd64/arm64) 이미지를 직접 풀:
     ...
     ```
     기본 이미지: `ghcr.io/brokermr810/fractal-{backend,frontend}:latest`. 양쪽을 동시에 고정하려면 로컬 `.env`에 `IMAGE_TAG=v3.0.9`, 한쪽만 고정하려면 `BACKEND_TAG` / `FRONTEND_TAG`.
   - **프론트엔드 로컬 개발**: `Fractal-Vue`를 `./Fractal-Vue/`(gitignore)에 클론 후 `docker compose -f docker-compose.yml -f docker-compose.build.yml up -d --build`. 자세한 내용은 [영어 README](../README.md#alternative-build-the-frontend-from-vue-source) 참고.
...
**상업적 이용?** 백엔드는 **Apache 2.0**. [Fractal-Vue](https://github.com/brokermr810/Fractal-Vue) 프론트는 별도 소스 가용 라이선스—상업 이용 전 조항을 확인하세요. 모바일은 [Fractal-Mobile](https://github.com/brokermr810/Fractal-Mobile) 라이선스를 따릅니다.

**모바일은?** [Fractal-Mobile](https://github.com/brokermr810/Fractal-Mobile)을 참고하세요.

## 거래소 추천 링크(참고)

| 거래소 | 링크 |
|--------|------|
| Binance | [가입](https://www.bsmkweb.cc/register?ref=FRACTAL) |
| OKX | [가입](https://www.xqmnobxky.com/join/FRACTAL) |
| Bybit | [가입](https://partner.bybit.com/b/DINGER) |

## 라이선스

- 백엔드: **Apache License 2.0** ([`../LICENSE`](../LICENSE))
- 동봉 Web UI: 프리빌드 배포. 소스는 [Fractal-Vue](https://github.com/brokermr810/Fractal-Vue)(별도 라이선스)
- 상표: [`../TRADEMARKS.md`](../TRADEMARKS.md)

## 면책

Fractal는 합법적인 연구·교육·규정 준수 거래용입니다. **투자 조언이 아닙니다.** 이용은 본인 책임입니다.
