...
# ============================================================
#
# Fractal's webhook channel was originally generic: it POSTed our
...
def _build_webhook_text(payload: Dict[str, Any]) -> Tuple[str, str]:
    ...
    if explicit_title or explicit_msg:
        return (explicit_title or 'Hades'), (explicit_msg or '')
    ...
    title = ' · '.join(title_bits) if title_bits else 'Hades 信号'
...
"""
    ...
    def _notify_webhook(
        ...
        *,
        ...
        URL hosts recognised automatically:
          ...
          - Slack       hooks.slack.com/services/...
          - Generic     everything else (keeps Fractal's own schema)
        ...
        headers: Dict[str, str] = {
            ...
            "User-Agent": "Fractal/1.0 (+https://www.fractal.com)",
        }
        ...
        if signing_secret and dialect == 'feishu':
            ...
            # Original Fractal contract: body-bound HMAC-SHA256 hex
    ...
    def send_profile_test_notifications(
        ...
        *,
        ...
        title = "Hades 通知测试" if zh else "Hades notification test"
        plain = (
            "这是一条来自 Hades 个人中心「通知设置」的测试消息。若您收到本条消息，说明该渠道配置正确。"
            if zh
            else "This is a test message from Hades profile notification settings. "
